...
func (c *currentSignServer) get() *config.SignServer {
	if len(base.Account.SignServers) == 1 {
		// 只配置了一个签名服务时不检查以及切换, 在get阶段返回，防止返回nil导致其他bug（可能）
		return &base.Account.SignServers[0]
	}
...
// signSubmit
// 提交回调 buffer
func signSubmit(uin string, cmd string, callbackID int64, buffer []byte) {
...
func signCallback(uin string, results []gjson.Result) {
	...
	for _, result := range results {
		...
		signSubmit(uin, cmd, callbackID, ret)
	}
}
